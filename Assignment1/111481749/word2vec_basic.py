...
def generate_batch(data, batch_size, num_skips, skip_window):
    ...
    if data_index > vocabulary_size:
        return [],[]
    
    currBatch = 0
    batchI, lblI =  0,0
    while currBatch < batch_size:
        curr_skips = 0
        ...
        for i in range(skip_window,0,-1):
#            if curr_skips >= num_skips or batchI >= batch_size or lblI >= batch_size:
            if curr_skips >= num_skips :
                break
            batch[batchI] = data[data_index]
            labels[lblI] =data[data_index-i]
            currBatch+=1
            curr_skips+=1
            batchI+=1
            lblI+=1
...
#            if curr_skips >= num_skips or batchI >= batch_size or lblI >= batch_size:
                break
            if data_index+i >=vocabulary_size:
                data_index = 0 
...
#         print()
        ...
        if step % checkpoint_step == 0:
            ...
            getMySim = ['first', 'american', 'would']
            for i, valid_word in enumerate(getMySim):
...
if __name__ == '__main__':
    ...
    print('Most common words (+UNK)',len(count),' ', count[:10])
    print('dictionary',len(dictionary),' ', dictionary.items()[:10])
    print('Sample data',len(data),' ', data[:10], [reverse_dictionary[i] for i in data[:10]])
...
#     num_sampled = 64    # Number of negative examples to sample.
    ...
    pretrained_model_path = './1pretrained/'

    checkpoint_model_path = './1checkpoints_%s/' % (loss_model)
    model_path = './models'
    
    # maximum training step
    max_num_steps = 150001
